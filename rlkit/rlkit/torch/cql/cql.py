...
class CQLTrainer(TorchTrainer):
    def __init__(
            ...
            # custom.
            max_value=None, # If the known max value is known provide here.
    ):
        super().__init__()
        self.env = env
        self.policy = policy
    ...
    def _get_policy_actions(self, obs, num_actions, network=None):
        ...
        new_obs_actions, new_obs_log_pi =\
                network(obs_temp).sample_and_logprob()
    ...
    def train_from_torch(self, batch):
        ...
        """
        cdist = self.policy(obs)
        policy_mean, policy_std = cdist.normal_mean, cdist.normal_std
        new_obs_actions, log_pi = cdist.rsample_and_logprob()
        ...
        new_next_actions, new_log_pi =\
                self.policy(next_obs).rsample_and_logprob()
        new_log_pi = new_log_pi.unsqueeze(-1)
        new_curr_actions, new_curr_log_pi =\
                self.policy(obs).rsample_and_logprob()
        new_curr_log_pi = new_curr_log_pi.unsqueeze(-1)
        ...
        ## add CQL
        random_actions_tensor = ptu.empty(q2_pred.shape[0] * self.num_random, actions.shape[-1]).uniform_(-1, 1)
