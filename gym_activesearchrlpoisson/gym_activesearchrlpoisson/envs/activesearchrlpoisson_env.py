...
class ActiveSearchRLPoisson(gym.Env):
    ...
    def __init__(self, lam: float=10, sigma2: float=0.05, num_agents: int=2, 
                 ...
                 log_space_resolution: int=100):
        super(ActiveSearchRLPoisson, self).__init__()
...
#        print(self.t)  
        self.t += 1
#        self.compress = (self.t*(self.num_agents*2) > self.num_hypotheses)
        self.compress = True
        ...
        for act in action.reshape((2, self.num_agents)).numpy():
            ...
            for point in self.ground_truth:
                if min(act) < point and max(act) > point:
                    observation += 1

            length = abs(min(act) - max(act))
            if length < 0.00001:
                print('Point action!')
            ...
            self.all_boundaries.append(min(act))
            self.all_boundaries.append(max(act))  
        self.all_boundaries = sorted(self.all_boundaries)                            # should we do something better here?
        ...
        b_len = abs(1-self.all_boundaries[-1])
        if b_len < 0.00001:
            print('Point hypothesis!')
        self.b_lengths.append(b_len)
        ...
        for dummy_action in self.actions:                                       # if we restrict the number of actions & observations to maintain, then this part will become cheaper
            ...
            for b_index, bin_ in enumerate(self.disjoint_bins):
                ...
                else:                                                           # May not need this bit
                    if min(dummy_action) <= bin_[0] and   \
                       max(dummy_action) >= bin_[1]:
                        indices.append(b_index)
            if len(indices) == 0:
        ...
        if True:                        # Each agent contributes two hypotheses for each action taken (assuming no two boundaries are the same, and ignoring whitespace)
            scores = []
        ...
        # Generate the belief representation
        self.b_dummies = self.generate_belief_rep()
    ...
    def generate_belief_rep(self):
        ...
        b_dummies = [int(x*self.lam) for x in self.b_lengths]

        for iteration in range(self.num_EA_iterations):                             # Should make this smaller so that it converges faster e.g. after delta is 1e-5
            ...
            for b_index in range(len(b_dummies)):
                ...
                if sum(p) > 0:
                    ...
                    p = np.zeros(self.upper_limit_N)
                    p[0] = 1
                for index, dummy_n in enumerate(range(self.upper_limit_N)):
                    mean += dummy_n * p[index]
                b_dummies[b_index] = mean
            if np.mean((np.asarray(b_dummies) - 
                np.asarray(b_dummies_before))**2) <= self.EA_tolerance:
                return b_dummies
        return b_dummies
    ...
    def log_gaussian_density(self, x, N, L, sigma2):
        ...
        sigma2 = sigma2*L 
        sig = np.sqrt(sigma2)
        return -1*np.log(np.sqrt(sig*2*np.math.pi)) - 0.5*((x - N)/sig)**2
